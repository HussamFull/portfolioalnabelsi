...
// تم تنظيف المصفوفة وحذف الكود المعلق تماماً لمنع حدوث خطأ الـ key المفقود
const menuList = [
 {
    id: 1,
    path: "/",
...
const Header = () => {
  ...
  const handleSubMenu = (id) => {
    setDropDownId(dropDownId === id ? null : id); // تعديل ذكي: لإغلاق القائمة المنسدلة عند الضغط عليها مرة أخرى
  };
        ...
        <div className="header-wrapper">
          ...
          <ul className={`main-menu ${menuActive ? "active" : ""}`}>
            {menuList.map(({ id, name, path, dropDown, section }) => {
              return (
                <li key={`menu-item-${id}`} onClick={() => handleSubMenu(id)}>
                  ...
                  {dropDown?.length > 0 && (
                    <ul className={`sub-menu ${dropDownId === id ? "sub-menu_active" : ""}`}>
                      {dropDown.map((subItem) => {
                        return (
                          <li key={`sub-item-${subItem.id}`}>
                            <HashLink smooth to={subItem.path}>
                              {subItem.name}
                            </HashLink>
                          </li>
                        );
